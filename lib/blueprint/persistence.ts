...
export async function persistBlueprint(projectId: string, userId: string, blueprintInput: ProductBlueprint) {
  ...
  const { data, error } = await supabase.from("projects").update({ plan: toJson(blueprint), plan_schema_version: blueprint.schemaVersion, generation_source: blueprint.generationSource, blueprint_version: blueprint.version, pressure_test: null, pressure_tested_at: null, pressure_test_blueprint_version: null, status: "planned" }).eq("id", projectId).eq("user_id", userId).select("*").maybeSingle();
