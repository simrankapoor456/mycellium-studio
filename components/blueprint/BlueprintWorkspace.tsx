...
export function BlueprintWorkspace({ projectId, initialBlueprint, facts }: { projectId: string; initialBlueprint: ProductBlueprint; facts: DiscoveryFact[] }) {
  ...
  return <div className="blueprint-workspace"><nav className="blueprint-mode" aria-label="Blueprint presentation"><button aria-pressed={mode === "blueprint"} onClick={() => setMode("blueprint")}>Blueprint view</button><button aria-pressed={mode === "document"} onClick={() => setMode("document")}>Structured document</button><span>v{blueprint.version} · {blueprint.generationSource}</span></nav>
    <section className="blueprint-overview" id="overview"><span className="eyebrow">Overview</span><h2>{blueprint.summary}</h2><dl><div><dt>Business objective</dt><dd>{blueprint.overview.businessObjective}</dd></div><div><dt>Target users</dt><dd>{blueprint.overview.targetUsers.join(", ")}</dd></div><div><dt>Success metrics</dt><dd>{blueprint.overview.successMetrics.join(", ") || "Needs review"}</dd></div></dl></section>
    <section className="blueprint-understanding" id="understanding"><div><span className="eyebrow">Understanding</span><h2>{blueprint.understanding.factIds.length} facts support this blueprint.</h2></div><div>{blueprint.understanding.unresolvedItems.length ? blueprint.understanding.unresolvedItems.map((item) => <span key={item}>{item}</span>) : <span>No blocking unresolved items.</span>}</div></section>
    ...
    <section className="blueprint-review" id="review"><span className="eyebrow">Review</span><h2>Quality score {blueprint.review.qualityScore}</h2>{blueprint.review.warnings.length ? blueprint.review.warnings.map((warning) => <p key={warning}>{warning}</p>) : <p>No generated warnings remain.</p>}</section>
    <section className="blueprint-exports" id="export"><div><span className="eyebrow">Export</span><h2>Export the edited persisted version.</h2></div><div><a href={`/api/projects/${projectId}/exports/markdown`}>Markdown</a><a href={`/api/projects/${projectId}/exports/json`}>JSON</a><a href={`/api/projects/${projectId}/exports/csv`}>CSV</a></div></section>
