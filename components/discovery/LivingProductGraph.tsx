...
import type { ProductGraph } from "@/lib/domain/discovery/schemas";

const CATEGORY_COLUMNS = ["business_objective", "problem", "target_users", "use_cases", "success_metrics", "functional_requirements", "non_functional_requirements", "constraints", "assumptions", "risks", "architecture_decisions", "unknowns"] as const;

export function LivingProductGraph({ graph }: { graph: ProductGraph }) {
  ...
  const positions = new Map(nodes.map((node, index) => {
    const column = Math.max(0, CATEGORY_COLUMNS.indexOf(node.category as (typeof CATEGORY_COLUMNS)[number]));
  ...
  return (
    <section className="living-graph" aria-label="Living Product Graph">
      ...
      <svg className="living-graph__svg" role="img" aria-label="Connected product understanding" viewBox="0 0 720 760">
        ...
        {nodes.map((node) => {
          ...
          return <g aria-label={`${node.label}: ${node.value}`} className="living-graph__node" data-selected={selected?.id === node.id} data-state={node.state} key={node.id} onClick={() => selectNode(node.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); selectNode(node.id); } }} role="button" tabIndex={0} transform={`translate(${position.x - 64} ${position.y - 25})`}><rect height="50" rx="9" width="128" /><text textAnchor="middle" x="64" y="21">{truncate(node.label, 18)}</text><text className="living-graph__node-state" textAnchor="middle" x="64" y="37">{node.state}</text></g>;
        })}
      </svg>
