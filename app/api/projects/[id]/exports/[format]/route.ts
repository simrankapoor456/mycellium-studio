...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string; format: string }> }) {
  ...
  if (!project.plan) return NextResponse.json({ error: "Generate a blueprint before exporting it." }, { status: 409 });
  ...
  if (!selected) return NextResponse.json({ error: "Export format not found." }, { status: 404 });
