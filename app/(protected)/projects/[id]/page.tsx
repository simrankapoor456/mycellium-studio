...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main>
    ...
    <header className="project-identity mt-5 border-b border-line pb-8"><div className="flex flex-wrap items-center gap-3"><Badge tone={project.status === "planned" || project.status === "ready" ? "success" : "warning"}>{project.status}</Badge><span className="text-sm text-ink/70">{getProductTypeLabel(projectType.success ? projectType.data : null, project.custom_project_type)}</span></div><h1 className="display-type mt-4 max-w-5xl break-words text-4xl text-forest sm:text-6xl">{project.name}</h1><p className="mt-5 max-w-3xl whitespace-pre-wrap text-lg leading-8 text-ink/65">{project.description || "Add source material to make the product context easier to review."}</p></header>
